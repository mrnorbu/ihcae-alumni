          ...
          <div class="space-y-3">
            ...
            @for (post of posts; track post; let i = $index) {
              <div class="space-y-3">
                <div class="bg-gradient-to-r from-blue-50 to-white border-l-4 border-blue-500 rounded-lg p-4 shadow-sm">
                  <div class="flex items-start gap-3">
                    ...
                    <div class="w-10 h-10 rounded-full bg-blue-100 flex items-center justify-center flex-shrink-0 text-sm font-bold text-blue-700 ring-2 ring-blue-200">
                      @if (post.author.profileImageUrl && !isLucideIconUrl(post.author.profileImageUrl)) {
                        <img
                ...
                @if ((localActiveReplyForm === post.id || activeReplyForm === post.id) && !topic.isLocked) {
                  <div class="ml-12 bg-blue-50 border-l-2 border-blue-400 rounded-lg p-3">
                    <div class="flex items-center gap-2 mb-2">
                    ...
                    <form (ngSubmit)="submitReply(post.id)">
                      ...
                      <div class="flex gap-2">
                        <button
                          ...
                          @if (!replySubmitting.has(post.id)) {
                ...
                @if (post.replies && post.replies.length > 0) {
                  <div class="ml-12 space-y-2">
                    ...
                    @for (reply of post.replies; track reply; let replyIndex = $index) {
                      <div class="relative">
                        <!-- Reply Card -->
                        <div class="bg-white border border-neutral-200 rounded-lg p-3 hover:shadow-md transition-all duration-200">
                          <div class="flex items-start gap-3">
                            ...
                            <div class="w-8 h-8 rounded-full bg-neutral-200 flex items-center justify-center flex-shrink-0 text-xs font-semibold text-neutral-600">
                              @if (reply.author.profileImageUrl && !isLucideIconUrl(reply.author.profileImageUrl)) {
                                <img
                        ...
                        @if ((localActiveReplyForm === reply.id || activeReplyForm === reply.id) && !topic.isLocked) {
                          <div class="ml-11 mt-2 bg-neutral-50 border-l-2 border-neutral-300 rounded-lg p-3">
                            <div class="flex items-center gap-2 mb-2">
                            ...
                            <form (ngSubmit)="submitReply(reply.id)">
                              ...
                              <div class="flex gap-2">
                                <button
                                  ...
                                  @if (!replySubmitting.has(reply.id)) {
                                    <span>Reply</span>
                                  }
                                  @if (replySubmitting.has(reply.id)) {
                                    <span>Posting...</span>
